...
| Tailwind 目标版本 | `--tw-version`       | `TW_VERSION`   | `3`                              | `tailwindcss` 开启时的目标 Tailwind 大版本，可选 `3` / `4`；`4` 走 [`css-to-tailwindcss4`](https://github.com/refinist/css-to-tailwindcss4)                                                                                                              |
...
| 工作目录          | `--cwd`              | `CWD`          | 进程 cwd                         | 显式覆盖 server 进程的工作目录；路径不存在或不可访问时启动阶段直接退出。部分客户端的 MCP 子进程工作目录不指向当前项目（如 TRAE / Codex / Qoder），必须在 `args` 里固定传入；值用编辑器内置变量 `${workspaceFolder}` 即可自动指向项目根，无需写死绝对路径 |
| 输出模式          | `--mode`             | `MODE`         | `inline`                         | 工具产出形态：`inline` 在 tool result 直接返回全部内容，`files` 落盘 `context.md` + `preview.png` 并只返回 `resource_link`                                                                                                                               |
| 产物目录          | `--out-dir`          | `OUT_DIR`      | `<cwd>/.lanhu-context-mcp.local` | `mode=files` 下设计稿上下文文件的产出目录；支持绝对或相对路径                                                                                                                                                                                            |
